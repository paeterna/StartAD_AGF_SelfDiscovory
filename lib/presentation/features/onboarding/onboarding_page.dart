...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  Future<void> _completeOnboarding(QuizInstrument metadata) async {
    ...
    try {
      ...
      for (final item in metadata.items) {
        if (item is ForcedChoiceItem) {
          for (final option in item.options) {
            featureMaxPossible[option.featureKey] =
                (featureMaxPossible[option.featureKey] ?? 0.0) + option.weight;
          }
        }
      }

      // Second pass: accumulate actual scores based on selections
      for (final item in metadata.items) {
      ...
      // For each feature, normalize based on the maximum possible score for THAT feature
      // Formula: (actual_score / max_possible_score) * 100
      // This correctly scales to 0-100 where:
      // - 0 selections = 0
      // - All selections = 100
      // - Proportional selections = proportional score
      final normalizedScores = <String, double>{};

      // Include ALL features that appeared in the assessment
      // Features with 0 selections get a score of 0
      for (final featureKey in featureMaxPossible.keys) {
        final actualScore = featureScores[featureKey] ?? 0.0;
        final maxPossible = featureMaxPossible[featureKey] ?? 1.0;

        // Normalize: (actual / max) * 100
        normalizedScores[featureKey] = (actualScore / maxPossible) * 100;
      }
