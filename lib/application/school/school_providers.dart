...
final searchSchoolsProvider = FutureProvider.family<List<School>, String>((ref, query) async {
  if (query.isEmpty) {
...
/// Get school KPIs
final schoolKpisProvider = FutureProvider.family<SchoolKpis, String>((ref, schoolId) async {
  final repository = ref.watch(schoolRepositoryProvider);
  return repository.getSchoolKpis(schoolId);
});

/// Get top students for school
final schoolTopStudentsProvider = FutureProvider.family<List<TopStudent>, String>((ref, schoolId) async {
  final repository = ref.watch(schoolRepositoryProvider);
  return repository.getTopStudents(schoolId);
});

/// Get career distribution for school
final schoolCareerDistributionProvider = FutureProvider.family<List<CareerDistribution>, String>((ref, schoolId) async {
  final repository = ref.watch(schoolRepositoryProvider);
  return repository.getCareerDistribution(schoolId);
});

/// Get school feature averages for radar
final schoolFeatureAveragesProvider = FutureProvider.family<List<SchoolFeatureAverage>, String>((ref, schoolId) async {
  final repository = ref.watch(schoolRepositoryProvider);
...
final schoolStudentsProvider = FutureProvider.family<List<SchoolStudent>, SchoolStudentsParams>((ref, params) async {
  final repository = ref.watch(schoolRepositoryProvider);
...
final schoolAssignmentControllerProvider = Provider((ref) {
  final repository = ref.watch(schoolRepositoryProvider);
